...
exports.buildAttachmentUri = buildAttachmentUri;
...
/**
 * Build attachment URI with access token parameter for authenticated access
 * @param contentUri - The base content URI of the attachment
 * @param accessToken - The access token for authentication (optional)
 * @returns The complete URI with access token if provided, otherwise the original URI
 */
function buildAttachmentUri(contentUri, accessToken) {
  if (!accessToken) {
    return contentUri;
  }
  return "".concat(contentUri, "?access_token=").concat(decodeURIComponent(accessToken), "&shouldCache=true");
}
function getMMSAttachments(message, accessToken) {
  ...
  return attachments.map(function (attachment) {
    var uri = buildAttachmentUri(attachment.uri, accessToken);
